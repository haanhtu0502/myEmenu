...
class ProductCategoryRepositories extends ApiCallHelper {
  ...
  Future<SResult<PaginationResponse<ProductModel>>> getProduct(
      {required GetProductRequest request}) async {
    return await apiCall<PaginationResponse<ProductModel>,
            PaginationResponse<ProductModel>>(
        mapper: (p0) => p0,
        request: () async =>
            await _productCategoryApi.getProduct(queries: request.toMap()));
  }
}
